import { DisplayObject, Graphics, Renderer, Ticker } from 'pixi.js';
import { Viewport, IViewportOptions } from 'pixi-viewport'; // use with modern build toolchain
...
export class CViewport extends Viewport{

    //viewport!:Viewport;
    renderer:Renderer;
    
    draggingWindowTF:boolean=false;
    draggingWindow:(DisplayObject | undefined)= undefined;

    constructor(renderer:Renderer,options:IViewportOptions){
        super(options);
    ...
    */
    this
        .drag({
            ...
            // mouseButtons: 'middle-right',             // changes which mouse buttons trigger drag, use: 'all', 'left', right' 'middle', or some combination, like, 'middle-right'; you may want to set viewport.options.disableOnContextMenu if you want to use right-click dragging
            // keyToPress: ["Space"],                // array containing https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/code codes of keys that can be pressed for the drag to be triggered, e.g.: ['ShiftLeft', 'ShiftRight'}
        ...
        .wheel({
             percent: 0.01,                // smooth the zooming by providing the number of frames to zoom between wheel spins
    ...
    border() {
        ...
        line.lineStyle(10, 0x3333FF).drawRect(0, 0, this.worldWidth, this.worldHeight)
    }
...
/*
import { IViewportOptions, Viewport } from 'pixi-viewport' // use with modern build toolchain
import { Renderer, Graphics } from 'pixi.js'
...
export class CViewport extends Viewport{
    ...
    constructor (options?: IViewportOptions | undefined) {
